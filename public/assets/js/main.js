...
function initGlitchBackground() {
  ...
  const animate = () => {
    // DISABLED: No longer updating letters - glitch is frozen
...
function initSynchronizedEffects() {
  ...
  // Start rotating text after 2 seconds, then every 2 seconds
  // Glitch updates are disabled - only rotating text continues
  setTimeout(() => {
    // Trigger rotating text first time
    rotateTextFunction();

    // Set up interval for rotating text only (glitch is frozen)
    setInterval(rotateTextFunction, 2000);
  }, 2000);
...
function createFallbackBackground() {
  ...
  function animate() {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const y =
        (Math.cos(time + i * 0.5) * canvas.height) / 2 + canvas.height / 2;
